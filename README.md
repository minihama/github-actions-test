## Dockerfile 동작 확인을 위한 설정 (v0.0.20)
  1. ci.yml 깃허브 액선 활성화 및 node docker 비활성화 처리 (완료)
  2. ci.yml에 패키지 생성 구문 추가 (완료)
  3. 버전 및 패키지 생성 구문의 잡 나누는 작업 (완료)
  4. 릴리즈 테스트 및 패키지 생성 테스트 (완료)
  5. docker pull로 동작 테스트 (완료)
  6. tag 에러로 meta job부분 수정 (완료)
  7. tag 에러로 /go-app 이미지 이름 삭제 (완료)
  8. tag 에러로 TAG 값 삭제 (완료)
  9. tag 에러로 이미지 명-서비스명 으로 수정 (완료)
  10. tag 에러로 SERVICE_NAME 수정 (완료)
